...
- (void)reset
{
    ...
    // add receive bytes (steven yan)
    [self setRequestDidReceiveBytesSelector:NULL];
...
- (void)request:(ASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
    ...
    //add delegate for downloadManager
    if ([self requestDidReceiveBytesSelector]) {
		[[self delegate] performSelector:[self requestDidReceiveBytesSelector] withObject:request withObject:[NSString stringWithFormat:@"%lld", bytes]];
    }
}
...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    // add receive bytes (steven yan)
    [newQueue setRequestDidReceiveBytesSelector:[self requestDidReceiveBytesSelector]];
...
// add receive bytes (steven yan)
@synthesize requestDidReceiveBytesSelector;

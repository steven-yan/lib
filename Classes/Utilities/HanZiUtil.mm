...
static NSString *pinyinFirstLetter =
...
@implementation HanZiUtil

//获取汉字首字母
+ (char)getPinYinFirstLetter:(NSString *)hanzi {
    hanzi = [hanzi stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    
    //容错
    if ([ChkUtil isEmptyStr:hanzi]) {
        return -1;
    }
    
    int index = [hanzi characterAtIndex:0] - HANZI_START;
    if (index >= 0 && index <= HANZI_COUNT) {
        return [pinyinFirstLetter characterAtIndex:index];
    } else {
        return [[hanzi lowercaseString] characterAtIndex:0];
    }
}
